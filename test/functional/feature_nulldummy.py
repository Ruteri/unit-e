...
class NULLDUMMYTest(UnitETestFramework):
    ...
    def run_test(self):
        ...
        ms_tx = self.create_transaction(self.nodes[0], coinbase_txid[0], self.ms_address, PROPOSER_REWARD - Decimal('0.1'))
        ms_txid = self.nodes[0].sendrawtransaction(bytes_to_hex_str(ms_tx.serialize_with_witness()), True)

        wit_ms_tx = self.create_transaction(self.nodes[0], coinbase_txid[1], self.wit_ms_address, PROPOSER_REWARD - Decimal('0.1'))
        ...
        test1tx = self.create_transaction(self.nodes[0], ms_txid, self.address, PROPOSER_REWARD - Decimal('0.2'))
        ...
        test2tx = self.create_transaction(self.nodes[0], wit_ms_txid, self.wit_address, PROPOSER_REWARD - Decimal('0.2'))
